...
export const placeOrder = async (req, res) => {
  ...
  try {
    const { items, couponCode, paymentMethod = "ONLINE" } = req.body;
    ...
    try {
      ...
      const message = `
        <h2>Thank You for Your Order!</h2>
        <p>Dear ${user.name},</p>
        <p>Your order has been received at <strong>My Canteen</strong>.</p>
        <ul>
          <li><strong>Items:</strong> ${orderItems}</li>
          <li><strong>Total:</strong> ₹${finalAmount}</li>
...
export const receiveCODPayment = async (req, res) => {
  try {
    ...
    res.json({ success: true, message: "COD payment marked as received" });
  } catch (error) {
    console.error("Error in receiveCODPayment:", error);
